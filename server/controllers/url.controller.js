...
const handleGetAllUrls = async (req, res) => {
    ...
    if (!all_redirect_urls)
        throw new ApiError(500, "Urls not found in the database");
...
const handleGenerateShortUrl = async (req, res) => {
    ...
    if (!redirectURL || !nanoId)
        throw new ApiError(500, "Redirect Url and Nano Id is required ");
    ...
    if (!newRedirectUrl)
        throw new ApiError(500, "New Url cannot not be created");
...
const handleRedirect = async (req, res) => {
    ...
    if (!redirect)
        throw new ApiError(
            400,
            "There is no redirect url for the given nanoId"
        );
